...
double Integral::Line_R1(const Point& observation, const Point& v0, const Point& v1)
{
	...
	double D0 = observation.distance(prj); //distance from observation to the line
	double s1, s0, R1, R0;
...
double Integral::Line_R_3(const Point& ob, const Point& v0,const Point& v1)
{
	double result = 0.0;
	Point e = (v1 - v0).getUnit();
	Point prj = ob.projection_line(v0, v1);
	...
	R1 = v1.distance(ob);
	if (abs(L0) < TOLERANCE){
		result = 0.5*abs(1./(s1*s1) - 1./(s0*s0)); // L0 = 0 and ob is not on Cij
	}
	else if (abs(L0) > TOLERANCE){
		result = (s1/(L0*L0*R1)) - (s0/(L0*L0*R0));
	}
	else{
		cout << "Site must not be on any edge!" << endl;
	}
	//cout << "Line_R_3:" << result << endl;//this  integral is proved to be right
	return result;
}

Point Integral::Line_rR_3(const Point& ob, const Point& v0,const Point& v1)
{
	Point result(0., 0., 0.);
	Point e = (v1 - v0).getUnit();
	//e.display();
	...
	R0 = v0.distance(ob);
	R1 = v1.distance(ob);
	result = L*Integral::Line_R_3(ob, v0, v1) + e*((1./R0) - (1./R1));
	//cout <<  " e*((1./R0) - (1./R1)): " << ((1./R0) - (1./R1)) << endl;
	return result;
}
...
double Integral::Surface_R_3_zyy(const Point& ob, const vector<Node*>& face_node, int n)
{
	double result = 0.0;
	//face_node.push_back(face_node[0]);
	Point unit_normal = unitCross(*face_node[1] - *face_node[0], *face_node[2] - *face_node[1]);
...
double Integral::Surface_R_5(const Point& observation, const vector<Node*>& face_node, int n)
{
	int nn = face_node.size(); //node number of the face
	assert(n == nn);
	double beta = getBeta(observation, face_node);//solid angle
	...
	for (int j = 0; j < n; j++) {
		...
		if(abs(mj_value) > TOLERANCE && abs(h) < TOLERANCE){
			bj_3 = (1./(3.*mj_value))*(((s1)/(pow(mj_value*mj_value+s1*s1,1.5))) - ((s0)/(pow(mj_value*mj_value+s0*s0,1.5)))) + ((2.)/(3.*mj_value*mj_value*mj_value))*(((s1)/(pow(mj_value*mj_value+s1*s1,0.5))) - ((s0)/(pow(mj_value*mj_value+s0*s0,0.5))));
		}
		else if (abs(mj_value) > TOLERANCE && abs(h) > TOLERANCE){
			bj_3 = (1./(abs(h)*abs(h)*abs(h)))*(atan((abs(h)*s1)/(mj_value*R1)) - atan((abs(h)*s0)/(mj_value*R0))) - ((mj_value)/((mj_value*mj_value+h*h)*h*h))*((s1/R1)-(s0/R0));
		}
		else {
			bj_3 = 0.0;
		}
	...
	//To reduce the singularity in formula when h = 0;
	if (abs(h) < TOLERANCE){
			result1 = - (1.0/3.0)*result; // + (1.0/3.0)*beta*(1./(abs(h)*h*h));
	}
	else{
		result1 = - (1.0/3.0)*result + (1.0/3.0)*beta*(1./(abs(h)*h*h));
	}
...
Point Integral:: Surface_rR_5(const Point& observation, const vector<Node*>& face_node, int n)
{
	Point unit_normal = unitCross(*face_node[1] - *face_node[0], *face_node[2] - *face_node[1]);
	double h = (observation - *face_node[0])*unit_normal;
	Point result(0, 0, 0);
	...
	for (int i = 0; i < n; i++) {
		next = (i + 1) % n;
		mj=unitCross(*face_node[next] - *face_node[i], unit_normal);
		result = result + mj*Line_R_3(observation, *face_node[i], *face_node[next]);
	}
		double sR_5 = Surface_R_5(observation, face_node, n);
		//cout << "surface_R_5" << sR_5 << endl;
		result =  (-1./3.)*result - unit_normal*h*sR_5;
	return result;
}

double Integral::Surface_x2R_5(const Point& observation, const vector<Node*>& face_node, int n)
{
	Point S_rR_5 = Surface_rR_5(observation, face_node, n);
	double S_R_3 = Surface_R_3(observation, face_node, n);
	Point unit_normal = unitCross(*face_node[1] - *face_node[0], *face_node[2] - *face_node[1]);
	double h = (observation - *face_node[0])*unit_normal;
	//face_node.push_back(face_node[0]);
	Point mj(0,0,0);
	...
	for (int i = 0; i < n; i++) {
		next = (i + 1) % n;
		mj = unitCross(*face_node[next] - *face_node[i], unit_normal);
		result = result +(mj.x)*(Line_rR_3(observation, *face_node[i], *face_node[next]).x);
	}
	result =  (- 1./3.)*result - (unit_normal.x)*h*(S_rR_5.x) + ((1. - (unit_normal.x)*(unit_normal.x))/3.)*S_R_3;
	return result;
}

double Integral::Surface_y2R_5(const Point& observation, const vector<Node*>& face_node, int n)
{
	Point S_rR_5 = Surface_rR_5(observation, face_node, n);
	//S_rR_5.display();/////////////////////////////////////////////////////////////////////////singularity
	double S_R_3 = Surface_R_3(observation, face_node, n);
	Point unit_normal = unitCross(*face_node[1] - *face_node[0], *face_node[2] - *face_node[1]);
	double h = (observation - *face_node[0])*unit_normal;
	//face_node.push_back(face_node[0]);
	...
	for (int i = 0; i < n; i++) {
		next = (i + 1) % n;
		mj = unitCross(*face_node[next] - *face_node[i], unit_normal);
		result = result - (1./3.)*(mj.y)*(Line_rR_3(observation, *face_node[i], *face_node[next]).y);
	}
	result = result - (unit_normal.y)*h*(S_rR_5.y) + ((1 - (unit_normal.y)*(unit_normal.y))/3.)*S_R_3;
	return result;
}

double Integral::Surface_z2R_5(const Point& observation, const vector<Node*>& face_node, int n)
{
	Point S_rR_5 = Surface_rR_5(observation, face_node, n);
	double S_R_3 = Surface_R_3(observation, face_node, n);
	Point unit_normal = unitCross(*face_node[1] - *face_node[0], *face_node[2] - *face_node[1]);
	...
	for (int i = 0; i < n; i++) {
		next = (i + 1) % n;
		mj = unitCross(*face_node[next] - *face_node[i], unit_normal);
		result = result - (1./3.)*(mj.z)*(Line_rR_3(observation, *face_node[i], *face_node[next]).z);
		
	}
	result = result - (unit_normal.z)*h*(S_rR_5.z) + ((1 - (unit_normal.z)*(unit_normal.z))/3.)*S_R_3;
	return result;
}

double Integral::Surface_xyR_5(const Point& observation, const vector<Node*>& face_node, int n)
{
	Point S_rR_5 = Surface_rR_5(observation, face_node, n);
	double S_R_3 = Surface_R_3(observation, face_node, n);
	Point unit_normal = unitCross(*face_node[1] - *face_node[0], *face_node[2] - *face_node[1]);
	...
	for (int i = 0; i < n; i++) {
		next = (i + 1) % n;
		mj = unitCross(*face_node[next] - *face_node[i], unit_normal);
		result = result - (1.0/3.0)*(mj.y)*(Line_rR_3(observation, *face_node[i], *face_node[next]).x);
	}
	result = result - (unit_normal.y)*h*(S_rR_5.x) + ((0 - (unit_normal.y)*(unit_normal.x))/3.)*S_R_3;
	return result;
}

double Integral::Surface_xzR_5(const Point& observation, const vector<Node*>& face_node, int n)
{
	Point S_rR_5 = Surface_rR_5(observation, face_node, n);
	double S_R_3 = Surface_R_3(observation, face_node, n);
	Point unit_normal = unitCross(*face_node[1] - *face_node[0], *face_node[2] - *face_node[1]);
	...
	for (int i = 0; i < n; i++) {
		next = (i + 1) % n;
		mj = unitCross(*face_node[next] - *face_node[i], unit_normal);
		result = result - (1./3.)*(mj.z)*(Line_rR_3(observation, *face_node[i], *face_node[next]).x);
	}
	result = result - (unit_normal.z)*h*(S_rR_5.x) + ((0 - (unit_normal.z)*(unit_normal.x))/3.)*S_R_3;
	//cout << "resultt_xz:: " << result << endl;
	return result;
}

double Integral::Surface_yzR_5(const Point& observation, const vector<Node*>& face_node, int n)
{
	Point S_rR_5 = Surface_rR_5(observation, face_node, n);
	double S_R_3 = Surface_R_3(observation, face_node, n);
	Point unit_normal = unitCross(*face_node[1] - *face_node[0], *face_node[2] - *face_node[1]);
	...
	for (int i = 0; i < n; i++) {
		next = (i + 1) % n;
		mj = unitCross(*face_node[next] - *face_node[i], unit_normal);
		result = result - (1./3.)*(mj.y)*(Line_rR_3(observation, *face_node[i], *face_node[next]).z);
	}
	result = result - (unit_normal.y)*h*(S_rR_5.z) + ((0 - (unit_normal.y)*(unit_normal.z))/3.)*S_R_3;
	return result;
}

/*
********************************************************To calculate the Line integral of  \int_Cij xpxq/R^3*(Integral Table 2.272.4)*************************************
*/
 double Integral::Line_x2R_3(const Point& ob, const Point& v0,const Point& v1)
 {
	double result = 0.0;
	Point e = (v1 - v0).getUnit();
	Point prj = ob.projection_line(v0, v1);
	...
	double Lx = L.x;
	double ex = e.x;
	//result = ex*ex*((s1/R1) - (s0/R0) + Integral::Line_R_1(ob,v0, v1)) + Lx*ex + Lx*ex*(1/R0 - 1/R1) + Lx*Lx*(Integral::Line_R3(ob, v0, v1));
	if (abs(L0) < TOLERANCE) {
 	//assert(s0*s1>0);
		result = ex*ex*(abs(log(s1/s0)))+ (Lx*ex + Lx*ex)*abs(1/s0 - 1/s1) + Lx*Lx*(0.5*abs(1/(s1*s1) - 1/(s0*s0)));
	}
	else {
		result = ex*ex*(-(s1/R1) + (s0/R0) + (log((s1+R1)/(s0+R0))))+ (Lx*ex + Lx*ex)*(1/R0 - 1/R1) + Lx*Lx*((s1/(L0*L0*R1)) - (s0/(L0*L0*R0)));
	} 
	//cout << "Line_x2R_3 = " << result <<endl;
	 return result;
 	
 }

double Integral::Line_y2R_3(const Point& ob, const Point& v0,const Point& v1)
 {
	 double result = 0.0;
	 Point e = (v1 - v0).getUnit();
	Point prj = ob.projection_line(v0, v1);
	...
	if (abs(L0) < TOLERANCE) {
 	//assert(s0*s1>0);
		result = ey*ey*( abs(log(s1/s0)))+ (Ly*ey + Ly*ey)*abs(1/s0 - 1/s1) + Ly*Ly*(0.5*abs(1/(s1*s1) - 1/(s0*s0)));
	}
	else {
		result = ey*ey*(-(s1/R1) + (s0/R0) + (log((s1+R1)/(s0+R0))))+ (Ly*ey + Ly*ey)*(1/R0 - 1/R1) + Ly*Ly*((s1/(L0*L0*R1)) - (s0/(L0*L0*R0)));
	} 
	//cout << "Line_y2R_3 = " << result <<endl;
	 return result;

 }

double Integral::Line_z2R_3(const Point& ob, const Point& v0,const Point& v1)
 {
	 double result = 0.0;
	 Point e = (v1 - v0).getUnit();
	Point prj = ob.projection_line(v0, v1);
	...
	if (abs(L0) < TOLERANCE) {
		//assert(s0*s1>0);
		result = ez*ez*(abs(log(s1/s0)))+ (Lz*ez + Lz*ez)*abs(1/s0 - 1/s1) + Lz*Lz*(0.5*abs(1/(s1*s1) - 1/(s0*s0)));
	}
	else {
		result = ez*ez*(-(s1/R1) + (s0/R0) + (log((s1+R1)/(s0+R0))))+ (Lz*ez + Lz*ez)*(1/R0 - 1/R1) + Lz*Lz*((s1/(L0*L0*R1)) - (s0/(L0*L0*R0)));
	}
	 return result;
 }

double Integral::Line_xyR_3(const Point& ob, const Point& v0,const Point& v1)
 {
	double result = 0.0;
	Point e = (v1 - v0).getUnit();
	Point prj = ob.projection_line(v0, v1);
	...
	if (abs(L0) < TOLERANCE) {
		//assert(s0*s1>0);
		result = ex*ey*(abs(log(s1/s0)))+ (Lx*ey + Ly*ex)*abs(1/s0 - 1/s1) + Lx*Ly*(0.5*abs(1/(s1*s1) - 1/(s0*s0)));
	}
	else {
		result = ex*ey*(-(s1/R1) + (s0/R0) + (log((s1+R1)/(s0+R0))))+ (Lx*ey + Ly*ex)*(1/R0 - 1/R1) + Lx*Ly*((s1/(L0*L0*R1)) - (s0/(L0*L0*R0)));
	}
	//cout << "Line_xyR_3 = " << result <<endl;
	 return result;
 }

double Integral::Line_xzR_3(const Point& ob, const Point& v0,const Point& v1)
 {
	double result = 0.0;
	Point e = (v1 - v0).getUnit();
	Point prj = ob.projection_line(v0, v1);
	...
	if (abs(L0) < TOLERANCE) {
		//assert(s0*s1>0);
		result = ex*ez*(abs(log(s1/s0)))+ (Lx*ez + Lz*ex)*abs(1/s0 - 1/s1) + Lx*Lz*(0.5*abs(1/(s1*s1) - 1/(s0*s0)));
	}
	else {
		result = ex*ez*(-(s1/R1) + (s0/R0) + (log((s1+R1)/(s0+R0))))+ (Lx*ez + Lz*ex)*(1/R0 - 1/R1) + Lx*Lz*((s1/(L0*L0*R1)) - (s0/(L0*L0*R0)));
	}
	 return result;
 }

double Integral::Line_yzR_3(const Point& ob, const Point& v0,const Point& v1)
 {
	 double result = 0.0;
	 Point e = (v1 - v0).getUnit();
	Point prj = ob.projection_line(v0, v1);
	...
	R1 = v1.distance(ob);
	double Lz = Lz, Ly = L.y;
	double ez = e.z, ey = e.y;
	if (abs(L0) < TOLERANCE) {
		//assert(s0*s1>0);
		result = ez*ey*(abs(log(s1/s0)))+ (Lz*ey + Ly*ez)*abs(1/s0 - 1/s1) + Lz*Ly*(0.5*abs(1/(s1*s1) - 1/(s0*s0)));
	}
	else {
		result = ez*ey*(-(s1/R1) + (s0/R0) + (log((s1+R1)/(s0+R0))))+ (Lz*ey + Ly*ez)*(1/R0 - 1/R1) + Lz*Ly*((s1/(L0*L0*R1)) - (s0/(L0*L0*R0)));
	}
	//cout << "Line_yzR_3 = " << result <<endl;
	 return result;
 }

 void Integral::Surface_f3R_5(const Point& ob, const vector<Node*>& face, int n,
		double& x3R_5, double& y3R_5, double &z3R_5,
		double &x2yR_5, double& x2zR_5,
		double &y2xR_5, double& y2zR_5,
		double &z2xR_5, double& z2yR_5,
		double &xyzR_5)
		{
			int nn = face.size();
			assert(n == nn);
			Point r_R3 = Surface_rR_3(ob,face,n);
			double x2R_5= Surface_x2R_5(ob, face, n);
			double y2R_5= Surface_y2R_5(ob, face, n);
			double z2R_5= Surface_z2R_5(ob, face, n);
			double xyR_5= Surface_xyR_5(ob, face, n);
			...
			Point mij (0.,0.,0.);
			Point unit_normal = unitCross(*face[1] - *face[0], *face[2] - *face[1]);
			double hi = unit_normal*(ob - *face[0]);
			for (int j = 0; j < n; j++)
			{
				int j_next = (j + 1) % n;
				mij = unitCross(*face[j_next] - *face[j], unit_normal);
				//mij.display();
				x2R_3dl = Line_x2R_3(ob, *face[j], *face[j_next]);
				//cout << "Line_x2R_3dl = " <<x2R_3dl[j] <<endl;
				y2R_3dl = Line_y2R_3(ob, *face[j], *face[j_next]);
				//cout << "Line_y2R_3dl = " <<y2R_3dl[j] <<endl;
				z2R_3dl = Line_z2R_3(ob, *face[j], *face[j_next]);
				xyR_3dl = Line_xyR_3(ob, *face[j], *face[j_next]);
			
				x3R_5 += (-1.0/3.0)*(mij.x)*x2R_3dl;
				y3R_5 += (-1.0/3.0)*(mij.y)*y2R_3dl;
				//cout << "y3R_5 = " << y3R_5 << endl;
				z3R_5 += (-1.0/3.0)*(mij.z)*z2R_3dl;
				x2yR_5 += (-1.0/3.0)*(mij.y)*x2R_3dl;
				x2zR_5 += (-1.0/3.0)*(mij.z)*x2R_3dl;
				y2xR_5 += (-1.0/3.0)*(mij.x)*y2R_3dl;
				y2zR_5 += (-1.0/3.0)*(mij.z)*y2R_3dl;
				z2xR_5 += (-1.0/3.0)*(mij.x)*z2R_3dl;
				z2yR_5 += (-1.0/3.0)*(mij.y)*z2R_3dl;
				xyzR_5 += (-1.0/3.0)*(mij.z)*xyR_3dl;
			} 
			//cout << "x3R_5 = " << x3R_5 << '\t' << "y3R_5 = " << y3R_5 << endl;

			x3R_5 += -(unit_normal.x)*hi*x2R_5 + (1.0/3.0)*(1 - (unit_normal.x)*(unit_normal.x))*r_R3.x +  (1.0/3.0)*(1 - (unit_normal.x)*(unit_normal.x))*r_R3.x;
			y3R_5 += -(unit_normal.y)*hi*y2R_5 + (1.0/3.0)*(1 - (unit_normal.y)*(unit_normal.y))*r_R3.y +  (1.0/3.0)*(1 - (unit_normal.y)*(unit_normal.y))*r_R3.y;
			z3R_5 += -(unit_normal.z)*hi*z2R_5 + (1.0/3.0)*(1 - (unit_normal.z)*(unit_normal.z))*r_R3.z +  (1.0/3.0)*(1 - (unit_normal.z)*(unit_normal.z))*r_R3.z;
			x2yR_5 += -(unit_normal.y)*hi*x2R_5 + (1.0/3.0)*(0 - (unit_normal.y)*(unit_normal.x))*r_R3.x +  (1.0/3.0)*(0 - (unit_normal.y)*(unit_normal.x))*r_R3.x;
			x2zR_5 += -(unit_normal.z)*hi*x2R_5 + (1.0/3.0)*(0 - (unit_normal.z)*(unit_normal.x))*r_R3.x +  (1.0/3.0)*(0 - (unit_normal.z)*(unit_normal.x))*r_R3.x;
			y2xR_5 += -(unit_normal.x)*hi*y2R_5 + (1.0/3.0)*(0 - (unit_normal.x)*(unit_normal.y))*r_R3.y +  (1.0/3.0)*(0 - (unit_normal.x)*(unit_normal.y))*r_R3.y;
			y2zR_5 += -(unit_normal.z)*hi*y2R_5 + (1.0/3.0)*(0 - (unit_normal.z)*(unit_normal.y))*r_R3.y +  (1.0/3.0)*(0 - (unit_normal.z)*(unit_normal.y))*r_R3.y;
			z2xR_5 += -(unit_normal.x)*hi*z2R_5 + (1.0/3.0)*(0 - (unit_normal.x)*(unit_normal.z))*r_R3.z +  (1.0/3.0)*(0 - (unit_normal.x)*(unit_normal.z))*r_R3.z;
			z2yR_5 += -(unit_normal.y)*hi*z2R_5 + (1.0/3.0)*(0 - (unit_normal.y)*(unit_normal.z))*r_R3.z +  (1.0/3.0)*(0 - (unit_normal.y)*(unit_normal.z))*r_R3.z;
			xyzR_5 += -(unit_normal.z)*hi*xyR_5 + (1.0/3.0)*(0 - (unit_normal.y)*(unit_normal.z))*r_R3.x +  (1.0/3.0)*(0 - (unit_normal.z)*(unit_normal.x))*r_R3.y;
			return;
		}

		double Integral::Surface_R_3(const Point& observation, const vector<Node*>& face_node, int n)
		{
	int nn = face_node.size(); //node number of the face
	assert(n == nn);
	double beta = getBeta(observation, face_node);//solid angle
	...
	for (int j = 0; j < n; j++) {
		...
		if(abs(mj_value) > TOLERANCE && abs(h) < TOLERANCE){
			bj_1 = (1./(mj_value))*(((s1)/(pow(mj_value*mj_value+s1*s1,0.5))) - ((s0)/(pow(mj_value*mj_value+s0*s0,0.5))));
		}
		else if (abs(mj_value) > TOLERANCE && abs(h) > TOLERANCE){
			bj_1 = (1./(abs(h)))*(atan((abs(h)*s1)/(mj_value*R1)) - atan((abs(h)*s0)/(mj_value*R0)));
		}
		else {
			bj_1 = 0.0;
		}
	...
	//To reduce the singularity in formula when h = 0;
	if (abs(h) < TOLERANCE){
			result1 = - result; // + (1.0/3.0)*beta*(1./(abs(h)*h*h));
	}
	else{
		result1 = - result + beta*(1./(abs(h)));
	}
	return result1;
		}