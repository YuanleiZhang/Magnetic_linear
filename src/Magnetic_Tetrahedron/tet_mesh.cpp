...
void tet_mesh::nodes_in(std::istream & node_stream)
{
	...
	double x, y, z;
	int marker;
	for (unsigned int i = 0; i<n_nodes; i++)  //in order to adapt the output of Gmesh, the iterator should begin from "1" here,and end with number of node  +1
	{
		// Check input buffer
		assert(node_stream.good());
...
void tet_mesh::tets_in(std::istream & ele_stream)
{
	...
	for (unsigned int i = 0; i<n_tets; i++) //in order to adapt the output of Gmesh, the iterator should begin from "1" here,and end with number of ele +1
	{
		assert(ele_stream.good());
